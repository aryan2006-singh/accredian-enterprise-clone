  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-16">
      <div className="rounded-3xl bg-brand-600 px-6 py-12 text-white sm:px-10">
        <p className="text-sm font-semibold uppercase tracking-wide text-brand-50">Who Should Join?</p>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          {audiences.map((audience) => (
            <div key={audience.title}>
              ...
              <p className="mt-1 text-sm text-brand-50">{audience.description}</p>
            </div>
          ))}
        </div>
      </div>
